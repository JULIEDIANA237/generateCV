...
const Template1 = ({ personalInfo, experiences, education, skills, languages, interests }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-8 p-6 sm:p-8 bg-white rounded-lg shadow-xl">
      <div className="p-8 border-4 border-blue-500 rounded-lg" ref={cvRef}>
        ...
        <div className="flex flex-col md:flex-row items-center mb-6">
          ...
          <div className="ml-4 text-center md:text-left">
            <h1 className="text-3xl sm:text-4xl font-semibold text-gray-800">{personalInfo.firstName} {personalInfo.lastName}</h1>
        ...
        {/* Experience Section */}
        <div className="mb-6">
          <h2 className="text-2xl sm:text-3xl font-bold text-blue-500 mb-4">Expériences</h2>
          {experiences.length > 0 ? (
            experiences.map((experience, index) => (
              <div key={index} className="mb-4">
                <div className="flex flex-col sm:flex-row justify-between mb-2">
                  <div className="text-gray-800 font-bold text-lg sm:text-xl">{experience.title}</div>
                  <div className="text-gray-600 text-sm sm:text-base">{experience.startDate} - {experience.endDate}</div>
                </div>
                <p className="text-gray-600 text-sm sm:text-base">{experience.company}</p>
                <p className="text-gray-600 text-sm sm:text-base">{experience.jobDescription}</p>
              </div>
            ))
          ) : (
